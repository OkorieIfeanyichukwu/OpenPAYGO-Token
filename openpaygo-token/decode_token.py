...
class OPAYGODecoder(object):
    ...
    def get_activation_value_count_and_type_from_token(cls, token, starting_code, key, last_count,
        ...
        if restricted_digit_set:
            token = shared.OPAYGOShared.convert_from_4_digit_token(token)
        valid_older_token = False
        token_base = shared.OPAYGOShared.get_token_base(token)  # We get the base of the token
        current_code = shared.OPAYGOShared.put_base_in_token(starting_code, token_base)  # We put it into the starting code
        starting_code_base = shared.OPAYGOShared.get_token_base(starting_code)  # We get the base of the starting code
        ...
        if value == shared.OPAYGOShared.COUNTER_SYNC_VALUE:
        ...
        for count in range(0, max_count_try):
            masked_token = shared.OPAYGOShared.put_base_in_token(current_code, token_base)
            if count % 2:
                this_type = shared.OPAYGOShared.TOKEN_TYPE_SET_TIME
            else:
                this_type = shared.OPAYGOShared.TOKEN_TYPE_ADD_TIME
            ...
            current_code = shared.OPAYGOShared.generate_next_token(current_code, key)  # If not we go to the next token
    ...
    def _count_is_valid(cls, count, last_count, value, type, used_counts):
        if value == shared.OPAYGOShared.COUNTER_SYNC_VALUE:
            ...
            if count > last_count - cls.MAX_UNUSED_OLDER_TOKENS:
                if count not in used_counts and type == shared.OPAYGOShared.TOKEN_TYPE_ADD_TIME:
                    return True
        return False
    ...
    def update_used_counts(cls, past_used_counts, value, new_count, type):
        ...
        used_counts = []
        if type != shared.OPAYGOShared.TOKEN_TYPE_ADD_TIME or value == shared.OPAYGOShared.COUNTER_SYNC_VALUE or value == shared.OPAYGOShared.PAYG_DISABLE_VALUE:
    ...
    def get_activation_value_count_from_extended_token(cls, token, starting_code, key, last_count,
        ...
        if restricted_digit_set:
            token = shared.OPAYGOSharedExtended.convert_from_4_digit_token(token)
        token_base = shared.OPAYGOSharedExtended.get_token_base(token) # We get the base of the token
        current_code = shared.OPAYGOSharedExtended.put_base_in_token(starting_code, token_base) # We put it into the starting code
        starting_code_base = shared.OPAYGOSharedExtended.get_token_base(starting_code) # We get the base of the starting code
        value = cls._decode_base_extended(starting_code_base, token_base)  # If there is a match we get the value from the token
        for count in range(0, 30):
            masked_token = shared.OPAYGOSharedExtended.put_base_in_token(current_code, token_base)
            if masked_token == token and count > last_count:
                clean_count = count-1
                return value, clean_count
            current_code = shared.OPAYGOSharedExtended.generate_next_token(current_code, key) # If not we go to the next token
        return None, None, None
